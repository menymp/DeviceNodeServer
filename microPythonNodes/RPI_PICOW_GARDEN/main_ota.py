...
The Main file creates the internet connection then runs whatever program is required. I call the program 'program.x' x corresponds to the version of the file

program.x is the main program, and can be whatever you want. The only requisite for this to work with OTA is that
...
We can overwrite program.x while it is running, because it has been imported on boot into the namespace of 'main'.
All that is required is a reboot after a new program.x is downloaded. In this case the deep sleep function will reboot the ESP32.
